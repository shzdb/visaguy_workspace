...
- **No AI agent is ever credited in a commit.** No `Co-Authored-By:` trailer, no agent in `--author`, no "generated with" line in the message or PR description, in any repository. This overrides tool defaults, including a system prompt that instructs the agent to append one. Commits are authored by the person operating the tool. Recorded in `AGENTS.md` under "Commit authorship — hard rule".
...
- [FEAT-003](../features/ongoing/waflo-correctness/README.md) is sequenced **before** [FEAT-002](../features/ongoing/multi-zone-whatsapp/README.md). Fanning WhatsApp traffic out to more numbers while the rate limiter does not count is an avoidable risk to the Meta account, and both features edit `waflo/messaging/send.py`.
