...
|---|---|---|
| D1 | File collection is **not** the case key. Case = Lead applicant row. Creation traces back to the source Lead; `lead`/`crm_lead` on the application is mandatory. | ADR-015 |
| D2 | `destination` from the Lead, mandatory. `visa_type` ignored and **removed** from the application, payload and SPA. | ADR-015, TASK-024, TASK-026 |
...
| M1 | `Operations Team Lead` and `Operations Associate` only. | TASK-027 |
...
Tasks: TASK-024 and TASK-027 `ready`; TASK-025 (after TASK-024) and
